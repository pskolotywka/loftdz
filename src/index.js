...
function findAllPSiblings(where) {
    let divs = where.childNodes;
    let result = [];

    for (let i = 0; i < divs.length - 1; i++ ) { 
        if (divs[i].nextElementSibling.tagName === 'P') {
            result.push(divs[i]);
        }
    }
        ...
        for (let i = 0; i < divs.length; i++) {
            ...
            if (item.nodeType == 3) {
                ...
                for (let i = 0; i < item.classList.length; i++) {
                    let classItem = item.classList[i];

                    if (classItem in obj.classes) {
                        obj.classes[classItem]++
                    } else {
                        obj.classes[classItem] = 1;
                    }
                }
                scanner(item)
            }
